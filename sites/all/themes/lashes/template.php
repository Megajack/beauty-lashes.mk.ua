...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for lashes
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('lashes_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function lashes_html_head_alter(&$head_elements) {
 ...
 */
function lashes_preprocess_search_block_form(&$vars) {
 ...
 */
function lashes_preprocess(&$vars, $hook) {
  $vars['lashes_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function lashes_preprocess_html(&$vars) {
  
  $vars['doctype'] = _lashes_doctype();
  $vars['rdf'] = _lashes_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = lashes_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = lashes_id_safe('section-' . $section);
 ...
 */
function lashes_preprocess_page(&$vars) {
 ...
 */
function lashes_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'lashes') . '/css/maintenance-page.css');
}
 ...
 */
function lashes_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function lashes_preprocess_block(&$vars, $hook) {
 ...
 */
function lashes_menu_tree($vars) {
 ...
 */
function lashes_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function lashes_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('lashes_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('lashes_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('lashes_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('lashes_breadcrumb_separator');
 ...
 * @return bool
 */
function lashes_tabs_float() {
  $float = (bool) theme_get_setting('lashes_tabs_float');
  $float_node = (bool) theme_get_setting('lashes_tabs_node');
...
function lashes_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function lashes_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = lashes_id_safe($element['#title']);
 ...
 */
function lashes_preprocess_menu_local_task(&$vars) {
 ...
 */
function lashes_menu_local_tasks(&$vars) {
  $output = '';
...
// function lashes_form_contact_site_form_alter(&$form, &$form_state, $form_id) {
//   $form['phone'] = array(
//     '#type' => 'textfield',
//     '#maxlength' => 60,
//     '#title' => 'Телефон'
...
// function lashes_mail_alter(&$message) {
